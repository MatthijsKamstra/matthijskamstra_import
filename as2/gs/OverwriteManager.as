...
class gs.OverwriteManager {	
	public static var version:Number = 0.961;
	...
	public static function manageOverwrites($instance:TweenLite, $objTweens:Object):Void {
		...
		var startTime:Number = $instance.startTime, a:Array = [], i:Number, p:String;
		for (p in $objTweens) {
			if (p != "info" && $objTweens[p] != $instance && $objTweens[p].startTime <= startTime) {
				a[a.length] = p;
			}
		}
		if (a.length == 0) {
