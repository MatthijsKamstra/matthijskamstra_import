...
* @author 	:  	Matthijs C. Kamstra [mck] & Niels de Keizer
* @version 	:	1.0 - class creation (AS3)
* @since 	:	21-11-2008 16:04 
* 
* Changelog:
* 		v 1.1 [08-12-2008] - added stuff from Niels
...
package nl.matthijskamstra.fonts  {
	...
	public class FontController extends EventDispatcher {
		...
		public function getFont(id:String)  : *	{				
			//trace( "FontController.getFont > id : " + id );
			...
			for (var item in fontArray) {
				if (fontArray[item].fontName == id) {
					realFontClass = fontArray[item];
					break;
				}
			}
			...
			if (!realFontClass){
				try {
					realFontClass = getFontClass(id);
				} catch (err:ReferenceError) {
					//There was an error...no such font found!
					return null;
				}
			}
			//trace( "realFontClass : " + realFontClass );
			return new realFontClass();
		}
