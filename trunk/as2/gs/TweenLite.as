...
class gs.TweenLite {
	...
	public function initTweenVals($hrp:Boolean, $reservedProps:String):Void {
		...
		if (this.target instanceof Array) {
			...
			for (p in this.vars) {
				if (p == "ease" || p == "delay" || p == "overwrite" || p == "onComplete" || p == "onCompleteParams" || p == "onCompleteScope" || p == "runBackwards" || p == "onUpdate" || p == "onUpdateParams" || p == "onUpdateScope" || p == "persist" || p == "volume" || (p == "timeScale" && !(this.target instanceof TweenLite)) || p == "onStart" || p == "onStartParams" || p == "onStartScope" || p == "renderOnStart" || p == "proxiedEase" || p == "easeParams" || ($hrp && $reservedProps.indexOf(" " + p + " ") != -1)) { 
